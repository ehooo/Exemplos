...
from Adafruit_GPIO.GPIO import get_platform_gpio
from Adafruit_GPIO.GPIO import IN
from Adafruit_GPIO.GPIO import PUD_UP
from Adafruit_GPIO.GPIO import BOTH
...
#
# Update ALERT/RDY pin implementation, add BBIO compatibility (https://github.com/adafruit/Adafruit_Python_GPIO) by Victor Torre, 12/2014
# ===========================================================================
...
class ADS1x15:
  ...
  # Constructor
  def __init__(self, address=0x48, ic=__IC_ADS1015, alert_pin=None, debug=False, pull_up_down=PUD_UP, rpimode=None):
    ...
    gpiokwargs = {}
    if rpimode:
      gpiokwargs['mode'] = rpimode
    self.gpio = get_platform_gpio(**gpiokwargs)
    self.set_alert_pin(alert_pin, pull_up_down=pull_up_down)
  ...
  def set_alert_pin(self, pin, pull_up_down=PUD_UP):
    # Set GPIO alert pin
    self.alert_pin = pin
    if pin:
      self.gpio.setup(pin, IN, pull_up_down=pull_up_down)
      self.gpio.add_event_detect(pin, BOTH, self.__alert_callback)
  def __alert_callback(self, pin):
    val = self.gpio.input(pin)
    if self.__callback:
      self.__callback(val)
    else:
      print "Alert event %s"%(val)
  def set_alert_callback(self, callback):
    # Add callback for manage ALERT/RDY pin events
    self.__callback = callback

  @property
  def alert(self):
        if not self.alert_pin:
          raise ValueError('Set alert_pin for read it')
        else:
          return self.gpio.input(self.alert_pin)
  @property
  def ready(self):
    return self.alert
  ...
  def readADCDifferential13(self, pga=6144, sps=250):
    ...
    see data sheet page 14 for more info. \
    The pga must be given in mV, see page 13 for the supported values."
    return self.readADCDifferential(1, 3, pga, sps)  
